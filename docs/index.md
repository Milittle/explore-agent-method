...
**问题**：AI 编码智能体长期依赖的持久化项目上下文（CLAUDE.md 等）会因代码演进而逐渐过时、膨胀、缺失 gotchas，导致重复发现、错误假设、token 浪费和决策质量下降（"记忆腐烂"）。这是所有长期 AI 辅助开发项目的隐性成本。

**解决方案**：
1. **双循环维护机制** — 代码库驱动的量化审计循环（claude-md-improver：发现 → 6维度100分 rubric 评估 → 结构化报告 → 提议） + 会话驱动的即时捕获循环（/revise-claude-md：反思缺失 → 精炼 → 提议）
2. **报告先行 + 人类审批门控** — 永远先输出完整质量报告和精确 diff+why，所有持久化写入必须用户显式批准，防止静默损坏
3. **严格价值过滤器** — 只添加"非显而易见、项目特定、可行动、可复现"的高价值信息；建立清晰的 TO ADD / NOT TO ADD 决策表（避免 obvious/generic/one-off/冗长）
4. **分层文件 + 模板验证** — 项目级（git共享）、.claude.local.md（个人+gitignore）、全局、包级四层清晰隔离；所有更新必须用真实 FS/命令交叉验证 currency

**关键价值**：为 Superpowers（设计门控+审查）、GSD（plan/verify 产出）、Claude Skills（常驻约束）、上下文工程（持久化层）、Vibe Coding 工程前置决策等方法论提供了可直接落地的"记忆基础设施"执行层。复杂度极低（插件即用）、回报即时，适用于所有 Claude Code 用户及团队。直接可用于本项目自身的技能与文档治理。
